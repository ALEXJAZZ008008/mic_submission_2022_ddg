...
\section{Methods} \label{sec:methods}
    ...
    \subsection{Select and Combine} \label{sec:select_and_combine}
        ...
        %\subsubsection{Selecting \glss{PC} Using A Neural Network} \label{sec:selecting_pcs_using_a_neural_network}
            In the second, a \gls{NN} was used for scoring ~\cite{Walker2020AutomaticAI}. The \gls{NN} is a pre-trained model designed to accept a signal as input and return a score between $0.0$ and $1.0$ where is most respiratory-like.% The network was originally trained on a similar set of training data where the scores were predetermined by clinicians.
...
\section{Results} \label{sec:results}
    ...
    %     \captionsetup{singlelinecheck=false, justification=centering}
    %     \caption{A plot showing the \gls{PC} used to generate the output signal (taken for the first acquisition of patient one).}
